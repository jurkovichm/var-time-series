...
Predicting more than one step ahead is done recursively in what can be referred to as the chain-rule of forecasting. Essentially, the *t+1* forecast value is used to predict the *t+2* value and so on. The *h*-step ahead forecast can be express as
...
- other types of models (VAR is basic, and we use a basic version of it.) ARCH and GARCH
